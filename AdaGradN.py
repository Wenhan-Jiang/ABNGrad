...
class AdaGradN(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, lr_decay=0, weight_decay=0, initial_accumulator_value=0):
        if not 0.0 <= lr:
        ...
        super(AdaGradN, self).__init__(params, defaults)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['max_std'] = torch.zeros_like(p.data, memory_format=torch.preserve_format)
                max_std = state['max_std']
                state['step'] += 1
                ...
                if grad.is_sparse:
                    ...
                    std = std.div(torch.norm(std), p=1) #norm
                    torch.max(max_std, std, out=max_std) #amsgrad
                    ...
                    state['sum'].div_(torch.norm(state['sum'], p=1))#norm
                    torch.max(max_std, state['sum'], out=max_std) #amsgrad
